---
type: event
status: candidate
canon_state: provisional
last_updated: 2026-05-15
audit_only: true
tags: [auto_ingested, ollama_cockpit_7_5A]
---
# Nexus Choice

Type: Event  
Status: candidate  
Canon State: provisional  
Last Updated: 2026-05-15

## Canon Summary
A pivotal decision by a Temporal Guardian that led to multiple contradictory outcomes in the timeline.

## Identity and Nature
Stub: Pending behavioral/worldbuilding definition.

## Relationships
- involved_in: Temporal Guardian

## Historical Role
Stub: Timeline and narrative function unresolved.

## Source Notes
- Auto-detected in raw/chapters/book_14_lessons_of_inheritance_075_anchor_points_of_time.md

## Unresolved Questions
- [ ] Confirm canonical status and arc placement
- [ ] Verify behavioral context and relationship predicates
- [ ] Add explicit line-level citations from source chapters
